...
self.addEventListener("install", function (event) {
  console.log("[Wingo PWA] Install Event processing");

  event.waitUntil(
    caches.open(CACHE_NAME).then(function (cache) {
      console.log("[Wingo PWA] Cached offline page during install");
  ...
  if (event.request.mode === "navigate") {
    event.respondWith(
      fetch(event.request).catch(function (error) {
        console.log("[Wingo PWA] Network request Failed. Serving offline page " + error);
